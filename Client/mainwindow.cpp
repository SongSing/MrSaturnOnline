...
{
    // expecting id, this is sent back as we join so we can grab our info that's generated by the server
...
void MainWindow::handleUserJoinedChannel(Packet p)
{
    ...
    if (this->hasChannel(c))
    {
        ...
        appendChannel(channelId, tr("<i>%1 <font color='%2'><b>%3</b></font> joined the channel!</i>").arg(timestamp(), color, name));
    }
}
